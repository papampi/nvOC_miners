...
function pluggable-installer {
  ...
  then
    ...
    wget --quiet --no-check-certificate --tries=0 --timeout=2 --wait=1 --show-progress -O "${pm_path}/$(jq -r .install.tarball "${pm}")" "$(jq -r .install.tarball_url "${pm}")" || pm_error=true
  fi
  ...
  then
    ...
    if [[ $CUDA_VER == $pm_rec ]]
    then
      stop-if-needed "$(realpath -s --relative-to="${NVOC_MINERS}" "${pm_path}/../recommended")"
      update-symlink "${pm_path}" ../recommended
    fi
    if [[ $(jq -r .install.latest "${pm}") == true ]]
    then
      stop-if-needed "$(realpath -s --relative-to="${NVOC_MINERS}" "${pm_path}/../latest")"
